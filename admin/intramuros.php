...
//echo login('medium', 'add-article', 'true');
...
if(@$_SERVER['PHP_AUTH_PW'] != @$GLOBALS['intramuros_login'])// PHP_AUTH_USER
...
$chemin_visuel = $GLOBALS['media_dir'].'/intramuros';
$racine = '../../../';
$new_width = 320;
...
$json = file_get_contents($GLOBALS['intramuros_flux']);// curl()
...
if(is_array($array))
{
	...
	if(@$_REQUEST['clean'])
	{
		...
		array_map('unlink', array_filter((array) glob(rtrim($_SERVER['DOCUMENT_ROOT'], '/').'/'.$GLOBALS['media_dir'].'/resize/intramuros/*')));
	...
	foreach($array as $key => $val) 
	{
		...
		if($verbose) echo '<hr><h2>'.$key.'. '.$val['title'].'</h2>';


		// construction de l'url
		$url = encode($val['title']);
		...
		if($img and @$val['image']) 
		{
			$visuel_source = $val['image'];
			...
			if($copy)
			{
				...
				if($source_width > $new_width) 
				{
					...
					// Resize
					$visuel_dest = resize($racine.$visuel_dest, $new_width, null, 'intramuros');	
		...
		$values = array(
			...
			'user_update' => 1,
			'date_update' => "'".date('Y-m-d H:i:s', strtotime(str_replace('-', '/', $val['last_modified'])))."'",
			'user_insert' => 1,
			'date_insert' => "'".date('Y-m-d H:i:s', strtotime(str_replace('-', '/', $val['published_at'])))."'"
		);
...
//echo '<br><br>'.str_replace('),','),<br><br>', $sql_content);
//exit(benchmark());


// // Suppression des tag lié au évènement intramuros
...
$GLOBALS['connect']->query("DELETE FROM ".$tc." WHERE type='article-intramuros'");// AND id<=0
if($GLOBALS['connect']->error) die($GLOBALS['connect']->error);
